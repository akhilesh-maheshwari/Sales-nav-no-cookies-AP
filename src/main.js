...
try {
  ...
  const trialStore  = await Actor.openKeyValueStore('boomerang-free-trials-sales-navigator');
  ...
  if (isFirstTime) {
    ...
    console.log(`\n🎁 First-time user! ${FREE_TRIAL_LEADS} free leads applied.`);
  } else {
  ...
  while (!batchJobs || batchJobs.length === 0) {
    ...
    if (retryCount > MAX_RETRIES) {
      throw new Error(`No batch slots available after ${MAX_RETRIES} retries. Please try again later.`);
    }
    console.log(`⏳ No slots available. Waiting 2 mins before retry ${retryCount}/${MAX_RETRIES}...`);
      ...
      if (rowsPushed > 0) {
        ...
        if (chargeableLeads > 0) {
          ...
          try {
            ...
            console.log(`👉 Add funds at apify.com/billing and re-run.`);
